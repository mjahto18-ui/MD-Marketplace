...
function getSupabase() {
  const url = process.env.export const dynamic = "force-dynamic";
import { createClient } from '@supabase/supabase-js';
...
export async function getGlobalConfig() {
  try {
    ...
    const { data: rows, error } = await supabase.from('md_global_control').select('*').limit(20);

    if (error) throw error;

    let cfg = {};
    (rows||[]).forEach(r => {
      const kRaw = (r['Key'] || '').toString().trim();
      if(!kRaw) return;
      ...
      const d = (r['Active_From'] || '').toString().trim();
    ...
    // === هون التصليح الوحيد - COMING_SOON ما بيسكر ===
    const isCartClosedByFlag = cfg['CART_ENABLED']?.value === 'FALSE';
    const isEmergency = cfg['EMERGENCY_LOCK']?.value === 'TRUE';
    const isClosedByTime =!isCartInHours;

    const result = {
      rawConfig: cfg,
      isCartClosed: isCartClosedByFlag || isEmergency || isClosedByTime,
