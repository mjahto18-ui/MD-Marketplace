...
let dataCacheTime = 0;
let dataFetchPromise = null; // <-- الحل للـ 2 اللي ضلو
...
export async function getCachedProductsAndStores() {
  ...
  // اذا في حدا عم يجيب البيانات حاليا، لا تروح تجيب مرة تانية، استناه
  if (dataFetchPromise) {
  ...
  dataFetchPromise = (async () => {
    try {
      ...
      dataFetchPromise = null; // فضي البromise بعد ما يخلص
    }
  })();
