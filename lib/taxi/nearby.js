...
export async function getNearbyDrivers(supabase, { origin_lat, origin_lng, vehicle_type, radiusKm = 5, minWallet = 50000 }) {
  ...
  if (wErr) {
    console.log('wallet filter error', wErr.message);
    // اذا جدول wallets مش موجود او فيه مشكل، رجع السواقين بدون فلتر محفظة
    return withDistance;
  }

  if (!wallets || wallets.length === 0) return [];

  const allowedIds = new Set(wallets.map(w => String(w.taxi_id)));
  return withDistance.filter(d => allowedIds.has(String(d.Taxi_ID)));
}
