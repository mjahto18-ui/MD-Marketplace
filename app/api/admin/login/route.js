...
export async function POST(req) {
  try {
    ...
    // دور بالـ Mobile او User ID مباشرة من DB
    const { data: users } = await supabase.from('users')
     .select('*')
     .or(`Mobile.eq.${phoneStr},User_x0020_ID.eq.${phoneStr},User_ID.eq.${phoneStr}`)
     .limit(5)

    let finalUser = users?.[0]

    // fallback للأدمن القديم 03177653
    ...
    // هلق مسموح: Admin, Store Owner, Driver, Assistant Admin, Accounting
    ...
    cookieStore.set('admin_session', JSON.stringify({
      ...
      name: finalUser.Name,
      phone: phoneStr,
      role: role, // هون منحفظ الدور الحقيقي
