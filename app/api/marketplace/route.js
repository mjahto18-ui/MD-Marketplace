...
// ✅ جديد: يرجع null اذا ما لقى سعرات
async function getCaloriesFromOFF(barcode) {
  try {
    ...
    if (data.status === 1 && data.product?.nutriments) {
      ...
      if (!n['energy-kcal_100g'] &&!n['energy-kcal_serving']) return null;
      let reply = `🔥 *السعرات الحرارية:*\n`;
...
function buildReply(p, caloriesText) {
  let base = `📦 *${p.name}*\n\n🏷 ${p.brand}\n🔢 ${p.code}\n⚖ ${p.quantity}\n🌍 ${p.countries}`;
  if (caloriesText) {
    base += `\n\n${caloriesText}`;
  } else {
    base += `\n\n⚠️ عذراً، لم نتمكن من العثور على السعرات الحرارية لهذا المنتج حالياً.`;
  }
  return base;
}
    ...
    if (waMessage) {
      ...
      if (CODE_INDEX.has(userText)) found = [CODE_INDEX.get(userText)];
        ...
        for (let i = 0; i < products.length; i++) {
          const p = products[i];
          if (p.nameLower.includes(q) || p.brandLower.includes(q) || p.code.includes(q)) {
            found.push(p);
...
لم نتمكن من العثور على المنتج "${userText}" ضمن قاعدة بياناتنا.

يمكنك المحاولة بـ:
• مسح الباركود مرة أخرى
• كتابة اسم المنتج بشكل أوضح
• إرسال صورة المنتج

نحن نعمل باستمرار على توسيع قاعدة البيانات لخدمتكم بشكل أفضل.`
        );
      } else {
        const p = found[0];
        const calories = await getCaloriesFromOFF(p.code);
        let reply = buildReply(p, calories);
    ...
    if (!found.length) {
      const notFound = `عذراً 🙏 لم نتمكن من العثور على المنتج "${query}" ضمن قاعدة بياناتنا. نعمل على توسيعها لخدمتكم بشكل أفضل.`;
    ...
    const p = found[0];
    const calories = await getCaloriesFromOFF(p.code);
    const reply = buildReply(p, calories);
