...
export async function GET(req) {
  try {
    ...
    const cust = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Customers!A:Z" });
    let wallet = 0;
    ...
    if(found) wallet = Number(found[found.length-1] || found[20] || 0); // اخر عمود غالبا هو الرصيد
