...
async function getCaloriesFromNet(barcode, productName) {
  ...
  try {
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/modelsgemini-3.5-flash-lite:generateContent?key=${GEMINI_API_KEY}`, {
      method: "POST",
      headers: {"Content-Type": "application/json" },
      body: JSON.stringify({
...
async function getCustomerRowByPhone(whatsappNumber) {
  try {
    const supabase = getSupabase();
    ...
    for (const c of data||[]) {
      ...
      if (m === phoneNorm || m.endsWith(phoneNorm.slice(-8))) return c;
    }
  } catch(e){}
  return null;
}
...
async function openBot3Session(phone) {
  ...
  const { data: existing } = await supabase.from('bot_sessions').select('*').eq('Phone', norm).maybeSingle();
  if (existing) {
    await supabase.from('bot_sessions').update({ "Active Bot": BOT3_SESSION, Status: "ACTIVE", "Last Activity": now, "Closed At": "" }).eq('Phone', norm);
  } else {
    await supabase.from('bot_sessions').insert([{ Phone: norm, "Active Bot": BOT3_SESSION, Status: "ACTIVE", "Started At": now, "Last Activity": now, "Request ID": "", "Closed At": "", data: {} }]);
  }
  return { ok: true };
}
...
async function transferToBot3({ from, user, originalMessage }) {
  ...
  const custRow = await getCustomerRowByPhone(from);
  if (!custRow) {
...
7.1 اذا قال "بدي اطلب / بدي اوردر / بدي اعمل طلب / بدي اشتري" والمستخدم موجود في Users بس Customer ID فاضي او غير موجود او "غير موجود":
   قل: "لقيت اسمك يا ${user?.name || ''} بس بعدك مش مسجل كزبون 😊 حتى اقدر بلشلك الأوردر، كمل تسجيلك السريع على موقعنا ${WEBSITE_URL} وبس تخلص قلي شو حابب تطلب وانا جاهز دغري"
   ممنوع منعاً باتاً تنقله الى BOT2 وممنوع تشرح PayPal او بطاقة ائتمان!
...
- اذا {Open Time} او {Close Time} فاضي: اعرض ⏰ الدوام: غير محدد ولا تعرض سطر 🕒 الحالة أبداً
...
export async function POST(req) {
  try {
    ...
    if (newOrderIntent) {
      ...
      const custRow = await getCustomerRowByPhone(whatsappNumber);
      if (!custRow) {
