...
async function getSheetRows(sheetName) {
  ...
  if (useCache) {
    ...
    if (loading) {
      console.log(`⏳ انتظار قراءة جارية: ${sheetName}`);
      try { return await loading; } catch (error) { console.error(`❌ فشل الطلب المشترك: ${sheetName}`, error.message); return []; }
    }
  }
  ...
  const loadPromise = (async () => {
    try {
      ...
      if (!rows.length) { console.log(`⚠ جدول ${sheetName} فارغ`); return []; }
...
async function getUserByWhatsAppNumber(whatsappNumber) {
  ...
  for (const row of rows) {
    ...
    if (rowWhatsApp === normalized) {
      ...
      console.log("🎯 تم التعرف على المستخدم:", JSON.stringify(user)); return user;
    }
  }
  console.log(`⚠ الرقم ${normalized} غير موجود في Users`); return null;
}

// ======================================================
// 7. البحث عن Products - مصحح
// ======================================================
...
async function searchProducts(userMessage) {
  ...
  for (const product of products) {
    ...
    let score = 0;
    for (const word of words) { if (productName.includes(word)) score += 2; }
    if (normalizeText(message).includes(productName)) score += 5;
    if (score <= 0) continue;

    const storeIdRaw = String(product["Store ID"] || "").trim();
    const storeIdNorm = storeIdRaw.toLowerCase();
    const store = stores.find(s => String(s["Store ID"] || "").trim().toLowerCase() === storeIdNorm);

    const categoryId = String(product["Category"] || "").trim();
    const category = categories.find(c => String(c["Category ID"] || "").trim() === categoryId);
    const areaId = String(product["Area"] || store?.["Area"] || "").trim();
    const area = areas.find(a => String(a["Area ID"] || "").trim() === areaId);
...
6. لا تخترع أي طلب. 7. لا تخترع أي سعر. 8. لا تخترع أي منتج. 9. لا تخترع أي متجر. 10. لا تخترع أي منطقة. 11. لا تخترع أي حالة طلب.

قواعد الموقع والايميل
- اذا سأل "شو موقعكم / وين العنوان / شو عنوانكم / وين محلكم / رابط الموقع / الموقع / العنوان": جاوب "موقعنا هو ${WEBSITE_URL} فيك تشوف كل المنتجات والفروع هناك"
- اذا سأل "كيف بقدر اتواصل / شو ايميلكم / بدي احكي الادارة / كيف احكيكم / الايميل": جاوب "فيك تتواصل معنا على ${INFO_EMAIL}"
...
قواعد المنتجات - مهم جدا
- إذا نفس اسم المنتج موجود في اكتر من متجر في productContext، لازم تذكر كل المتاجر، لا تقل نتيجة واحدة. مثال: "لقيت Candia بمحلين: سوبرماركت A بـ 75,000 و سوبرماركت C بـ 83,000"
- ممنوع تربط منتج بمتجر مش موجود بالـ productContext، اذا الـ Store ID مش بـ productResults لا تذكره. متجر B (مخبر B) ما بيبيع Candia، لا تذكره اذا مش موجود بالنتائج.
- إذا كانت بيانات المنتجات تحتوي على نتيجة مؤكدة استخدم: Product Name, Price, Store Name, Category, Address, Area ولا تظهر Product ID, Store ID, Area ID, Category ID
- إذا لم توجد نتيجة مؤكدة قل للمستخدم إنك لم تجد المنتج حالياً.

قواعد الطلبات
إذا سأل المستخدم عن طلب استخدم بيانات Order Request. يمكنك ذكر Request ID, Delivery Status, Approval Status, Items Cost, Delivery Fee, Total Amount, Delivery Address, Assigned Driver, Area لكن لا تعرض أي IDs داخلية غير ضرورية.
إذا سأل "شو بقلب الطلب؟" استخدم Order Details وحوّل Product ID → Product Name و Store ID → Store Name و Area → Area Name
حالة الطلب Delivery Status مهم جداً. اذا كانت في الطريق أو بيك اب أو تم التوصيل استخدم الحالة كما هي أو بصياغة لبنانية واضحة. لا تغير حالة الطلب من عندك.
إذا كان Assigned Driver موجوداً يمكن ذكر اسم السائق. لا تذكر رقم هاتف السائق إلا إذا طلب المستخدم ذلك بشكل واضح.

أسلوب المحادثة
لا تعيد الترحيب في كل رسالة. اذا شفت بالمحادثة السابقة انك قلت مرحبا قبل، لا تعيدها.
لا تقل "أنا ذكاء اصطناعي" إلا إذا سأل المستخدم. لا تقل "حسب البيانات التي لدي..." في كل رد. أجب مباشرة.
...
`;
    ...
    const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "llama-3.1-8b-instant", messages: [{ role: "system", content: systemPrompt }, { role: "user", content: userMessage }], temperature: 0.4 })
    });
    const data = await res.json();
    if (data.error) { console.error("❌ Groq Error:", JSON.stringify(data.error)); return "عذراً، صار عندي مشكلة صغيرة. جرب تبعتلي مرة تانية."; }
...
export async function POST(req) {
  try {
    ...
    if (user) { console.log(`👤 المستخدم: ${user.name} | Role: ${user.role} | Customer ID: ${user.customerId}`); }
    else { console.log("⚠ المستخدم غير موجود في Users"); }
    ...
    if (user) {
      ...
      orderContext = await buildOrderContext(user, userText);
    }

    const history = await getConversationHistory(from);
    const aiReply = await getAIReply(userText, user, productResults, orderContext, history) || "أهلا! أنا مساعدك الذكي من MD-Marketplace 😊 كيف بقدر ساعدك؟";
