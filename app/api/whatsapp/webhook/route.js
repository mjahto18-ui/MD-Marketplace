...
// DATE HELPER
// IMPORTANT:
// نستخدم YYYY-MM-DD حتى ما ينقلب اليوم والشهر
// ======================================================
...
async function transferToBot2(
  ...
  // ----------------------------------------------------
  // STEP 5
  // تغيير Session فقط بعد نجاح BOT2
  // ----------------------------------------------------
...
// IMPORTANT:
// صار يشتغل للضيف وللمستخدم المسجل
// ======================================================
...
async function getAIReply(
  ...
  try {
    ...
    let userContext =
      "المستخدم غير مسجل لدينا حالياً.";
...
1. إذا كان المستخدم معروفاً استخدم اسمه أحياناً وبشكل طبيعي.
...
6. لا تخترع أي متجر.
7. لا تخترع أي منطقة.
8. لا تخترع أي حالة طلب.
9. لا تعطِ معلومات غير موجودة في البيانات الموثوقة.
10. لا تكن رسمياً أو جافاً مع المستخدم.
11. جاوب حسب السؤال مباشرة.
12. لا تعيد الترحيب في كل رسالة.
13. لا تقل "أنا ذكاء اصطناعي" إلا إذا سأل المستخدم.
14. إذا السؤال غير واضح اسأل سؤال توضيحي واحد فقط.

==================================================
مهم جداً - المستخدم غير المسجل
==================================================

حالة المستخدم الحالية:
${isGuest ? "غير مسجل" : "مسجل"}

إذا كان المستخدم غير مسجل:

- مسموح له يسأل عن الموقع.
- مسموح له يسأل عن المنتجات.
- مسموح له يسأل عن الأسعار الموجودة في نتائج المنتجات.
- مسموح له يسأل عن المتاجر.
- مسموح له يسأل عن الخدمات.
- مسموح له يسأل عن العروض والمزايا.
- مسموح له يسأل أي استفسار عام عن MD-Marketplace.
- جاوبه بشكل طبيعي ومفيد حسب السؤال.

لكن:

ممنوع تنفيذ أو إنشاء أو تسجيل أي طلب للضيف.

إذا قال إنه يريد الطلب أو الشراء أو تسجيل طلب:

جاوبه بلطف، مثلاً:

"أكيد 😊 فيك تطلب من MD-Marketplace، بس كرمال نقدر نسجّل طلبك ونضمنلك تجربة سلسة، يرجى تسجيل الدخول أو إنشاء حساب على موقعنا:
${WEBSITE_URL}

وبعد التسجيل فيك ترجع تطلب مباشرة 🛒"
...
ولا ترسل المستخدم الضيف إلى BOT2.
...
جاوب:
"موقعنا هو ${WEBSITE_URL} فيك تشوف كل المنتجات والمتاجر والعروض هناك 😊"
...
إذا توجد نتائج مؤكدة اعرضها كما هي.

إذا لا توجد نتائج مؤكدة:
لا تخترع نتيجة.
...
لا تستخدم بيانات Order Request معه أبداً.

إذا المستخدم مسجل:
استخدم فقط الطلبات الموجودة في Order Request الخاصة به.

لا تسمح للمستخدم بالوصول إلى طلبات شخص آخر.
...
إذا سأل المستخدم سؤالاً بسيطاً، جاوب مباشرة.

إذا سأل عن منتج:
استخدم نتائج المنتجات.

إذا سأل عن الموقع:
أعطه الموقع.

إذا سأل عن الخدمات:
اشرح له حسب المعلومات المتوفرة.

إذا سأل عن الطلب وكان مسجلاً:
استخدم بيانات طلباته.

إذا سأل عن إنشاء طلب وكان غير مسجل:
اطلب منه التسجيل بطريقة لطيفة.
...
نتمنى لك تجربة تسوق ممتعة! 😊`;
    ...
    if (
      ...
      const guestOrderReply =
        `أكيد 😊 فيك تطلب من MD-Marketplace، بس كرمال نقدر نسجّل طلبك ونضمنلك تجربة سلسة، يرجى تسجيل الدخول أو إنشاء حساب على موقعنا:

${WEBSITE_URL}

وبعد التسجيل فيك ترجع تطلب مباشرة 🛒❤️`;

      console.log(
        "👤 ضيف حاول إنشاء طلب — طلبنا منه التسجيل"
      );
      ...
      await saveToAppSheet(
...

        {
          ...
          messageType:
            "GUEST_ORDER_REGISTRATION_REQUIRED"
        }
      );
    ...
    if (
      ...
      console.log(
        "🚀 المستخدم المسجل يريد إنشاء طلب جديد"
      );
