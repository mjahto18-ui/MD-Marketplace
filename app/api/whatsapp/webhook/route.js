...
// ======================================================
// 13. Groq AI - المحدث حسب الاتفاق الجديد
// ======================================================
...
async function getAIReply(
  ...
  try {
    let userContext =
      "المستخدم Guest غير مسجل - ما عندو طلبات - مسموح يشوف جدول Products فقط ليعرف اذا المنتج موجود ولا لا - ممنوع يقول منتج موجود وهو مش موجود";
...
هذا المستخدم مسجل، اعطيه خدمة كاملة متل القديم
`;
    }
    ...
    const productContext =
      productResults.length
       ? JSON.stringify(
        ...
        : "لا توجد نتائج منتجات مؤكدة - المنتج مش موجود في جدول Products";
...
- للدعم والشكاوى: ${SUPPORT_EMAIL}
- للمبيعات والاسعار: ${SALES_EMAIL}
...
1. إذا كان المستخدم معروفاً (مسجل)، استخدم اسمه عند الحاجة واعطيه خدمة كاملة متل المنطق القديم.

2. لا تنادِ المستخدم برقم الهاتف.

3. إذا كان المستخدم Guest غير مسجل:
   - رحب فيه بشكل طبيعي وودود: "أهلا وسهلا! كيف بقدر ساعدك؟"
   - هو ما عندو طلبات، فاذا سأل عن طلبي، قل "انت مش مسجل عنا حاليا، لمتابعة الطلبات سجل على ${WEBSITE_URL}"
   - مسموح له يشوف جدول Products فقط ليعرف اذا المنتج موجود ولا لا
   - ممنوع تقول منتج موجود وهو مش موجود في productContext - التزم بالجدول فقط
   - اذا سأل عن الموقع: عطيه ${WEBSITE_URL}
   - اذا سأل كيف اتواصل:
     * بدو يشتري / اسعار / منتجات → ${SALES_EMAIL}
     * عندو مشكلة بطلب / توصيل → ${SUPPORT_EMAIL}
     * سؤال عام → ${INFO_EMAIL}
   - خليك تاخد وتعطي معو لتفهم شو بدو، لا تقل جملة "سياسة الشركة" الناشفة
   - اذا بدو يعمل طلب او سعر دقيق: قل بلطف "كرمال اعطيك السعر الدقيق وتعمل طلب، بتحتاج تسجل على موقعنا ${WEBSITE_URL}"
...
8. لا تخترع أي منتج - اذا productContext يقول لا يوجد نتائج، قل المنتج مش موجود حاليا وشوف موقعنا ${WEBSITE_URL}
...
قل للمستخدم إنك لم تجد المنتج حالياً وشوف موقعنا ${WEBSITE_URL} او تواصل مع ${INFO_EMAIL}
...
رحب به بشكل طبيعي وودود.

اذا قال باي / شكرا / خلص / ما بدي شي:
انهي ب "شكرا لتواصلك مع MD-Marketplace! 🌸 نحن موجودين على ${WEBSITE_URL} ومع خدمة العملاء ${INFO_EMAIL} - يوم سعيد!"

اذا ما عرفت الجواب:
وجهه للموقع وخدمة العملاء: ${WEBSITE_URL} و ${INFO_EMAIL} / ${SUPPORT_EMAIL} / ${SALES_EMAIL}
...
نتائج المنتجات - هذا هو المرجع الوحيد لوجود المنتج
...
// ======================================================
// 15. WhatsApp POST - المحدث مع الفلاتر الجديدة
// ======================================================
...
export async function POST(req) {
  try {
    ...
    // ==================================================
    // NEW - فلتر statuses و nudge حسب الاتفاق
    // ==================================================
...
نتمنى لك تجربة تسوق ممتعة! 😊`;
    ...
    if (
      ...
      const endMsg =
        `شكرا لتواصلك مع MD-Marketplace! 🌸 اي وقت بتحتاج شي نحن موجودين على ${WEBSITE_URL} ومع خدمة العملاء ${INFO_EMAIL} - يوم سعيد!`;
      await sendMessage(
        whatsappNumber,
        endMsg
      );
    ...
    // مهم: الكل مسموح يشوف المنتجات حتى Guest - حسب الاتفاق
    ...
    // بس المسجل مسموح يشوف طلباتو - المنطق القديم نفسه
    if (user) {
      orderContext =
