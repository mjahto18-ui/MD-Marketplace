...
export async function GET(req) {
  ...
  try {
    ...
    const now = new Date();
    const nowHour = parseInt(new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Beirut', hour: '2-digit', hour12: false }).format(now));
    console.log(`NOW=${now.toISOString()} BeirutHour=${nowHour}`);
    ...
    for (const phone in lastMsg) {
      ...
      const diffHours = (now - last.date) / (1000 * 60 * 60);
      console.log(`CHECK ${phone} raw=${last.row["Date"]} diff=${diffHours.toFixed(2)} Sent=${last.row["Reassurance_Sent"]}`);
      if (isNaN(diffHours)) continue;
      if (diffHours < 12 || diffHours > 24) { console.log(`⏭ SKIP ${phone} diff ${diffHours.toFixed(2)} not in 12-24`); continue; }

      console.log(`--- ${phone} Reassurance_Sent=[${last.row["Reassurance_Sent"]}] Date=${last.row["Date"]} diffHours=${diffHours.toFixed(2)} ---`);
      if (String(last.row["Reassurance_Sent"] || "").toUpperCase() === "YES") { console.log(`⏭ SKIP ${phone} already YES`); continue; }
      ...
      await supabase.from('messages').update({ "Reassurance_Sent": "YES", "Reassurance_At": now.toISOString() }).eq('Message ID', last.row["Message ID"]);
      processed++;
    }
