...
export async function POST(req){
  try{
    ...
    // ⛔ ممنوع الإلغاء بعد قبول السائق
    if(['accepted','on_the_way','arrived','code_verified','in_progress'].includes(order.status)){
      return Response.json({error:'لا يمكن إلغاء الرحلة بعد قبول السائق - السائق في طريقه اليك'},{status:409});
    }
