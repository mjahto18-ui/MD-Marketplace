      ...
      <div className="max-w-6xl mx-auto p-4">
        ...
        {search ? (
          <div className="space-y-6">
            ...
            {searchResults.products.length > 0 && (<div><h2 className="text-white font-bold text-lg mb-3">📦 منتجات - {searchResults.products.length}</h2><div className="grid grid-cols-2 md:grid-cols-4 gap-3">{searchResults.products.slice(0, 8).map(product => (<div key={product.productID} className="glass rounded-xl p-3 active:scale-95 transition"><div className="relative w-full h-24 rounded-lg mb-2 bg-white/5 overflow-hidden"><Image src={product.image} alt={product.name} fill sizes="25vw" className="object-cover" loading="lazy" /></div><p className="text-white text-xs font-bold truncate">{product.name}</p><p className="text-pink-400 text-sm font-bold">{product.price} ل.ل</p></div>))}</div></div>)}
            {searchResults.stores.length > 0 && (<div><h2 className="text-white font-bold text-lg mb-3 mt-6">🏪 متاجر - {searchResults.stores.length}</h2><div className="grid grid-cols-2 md:grid-cols-4 gap-3">{searchResults.stores.slice(0, 8).map(store => (<Link key={store.store_id} href={`/store/${store.store_id}`} className="glass rounded-xl p-3 active:scale-95 transition"><div className="relative w-full h-20 rounded-lg mb-2 bg-white/5 overflow-hidden"><Image src={store.logo || '/placeholder.png'} alt={store.store_name} fill sizes="25vw" className="object-cover" loading="lazy" /></div><p className="text-white text-xs font-bold truncate">{store.store_name}</p></Link>))}</div></div>)}
            {searchResults.categories.length > 0 && (<div><h2 className="text-white font-bold text-lg mb-3 mt-6">📂 أقسام - {searchResults.categories.length}</h2><div className="grid grid-cols-3 md:grid-cols-5 gap-3">{searchResults.categories.map(cat => (<Link key={cat.id} href={`/category/${cat.id}`} className="glass rounded-xl p-3 text-center active:scale-95 transition"><div className="relative aspect-square bg-white/5 rounded-xl mb-2 overflow-hidden flex items-center justify-center p-2">{cat.image && (<Image src={cat.image} alt={cat.name} fill sizes="20vw" className="object-contain p-2" loading="lazy" />)}</div><p className="text-white text-xs font-semibold">{cat.name}</p></Link>))}</div></div>)}
          </div>
        ) : (
          <>
            <h2 className="text-white font-bold text-lg mb-4">تصفح حسب القسم</h2>
            <div className="grid grid-cols-3 md:grid-cols-5 gap-3 mb-8">
              {categories.map((cat) => (<Link key={cat.id} href={`/category/${cat.id}`} className="glass rounded-2xl p-3 text-center hover:bg-white/10 transition-all group active:scale-95"><div className="relative aspect-square bg-white/5 rounded-xl mb-2 overflow-hidden flex items-center justify-center p-2">{cat.image && (<Image src={cat.image} alt={cat.name} fill sizes="20vw" className="object-contain p-2 group-hover:scale-110 transition-all duration-300" loading="lazy" />)}</div><h3 className="text-white font-semibold text-xs">{cat.name}</h3></Link>))}
            </div>
          </>
        )}
      </div>
