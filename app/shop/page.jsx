...
export default function ShopPage() {
  ...
  useEffect(() => {
    // 1. لازم يكون في يوزر مسجل، والا رجعه عالـ login
    fetch('/api/me', {
      credentials: 'include',
      cache: 'no-store',
    })
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={handleBack} className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center hover:bg-white/20">
              ...
              <div>
                ...
                <p className="text-purple-200 text-xs">أهلاً {user?.name}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button onClick={() => window.location.href = '/dashboard'} className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center hover:bg-white/20" title="حسابي">
              ...
              <button onClick={handleLogout} className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center hover:bg-white/20" title="تسجيل خروج">
            ...
            <div key={i} className="glass rounded-2xl overflow-hidden">
              ...
              <div className="p-3">
                ...
                {user?.status === 'Active'? (
                  ...
                  <button className="w-full bg-white/10 text-white text-xs py-2 rounded-lg">
                    حسابك قيد المراجعة
                  </button>
                )}
              </div>
