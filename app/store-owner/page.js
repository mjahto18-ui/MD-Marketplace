    ...
    <div style={{minHeight:'100vh', background:'#0a1930', color:'white', fontFamily:'sans-serif'}}>
      <div style={{background:'#0e2242', borderBottom:'1px solid rgba(255,255,255,0.08)', padding:'10px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:10}}>
        <div style={{display:'flex', alignItems:'center', gap:10}}>
          ...
          <div><div style={{fontWeight:'900', fontSize:15}}>أهلاً {me.name}</div><div style={{fontSize:11, opacity:0.7}}>{store?.['Store Name'] || ''} - {me.storeId}</div></div>
        </div>
            ...
            <>
              ...
              {tab==='orders' && (
                <div style={{display:'grid', gap:10}}>
                  {preparing.length===0? <div style={{background:'#f3f1ec', color:'#1a1a1a', padding:20, borderRadius:12, textAlign:'center'}}>ما في طلبات<br/>details: {details.length} - reqOrders: {reqOrders.length} - storeId: {me.storeId}</div> :
                  preparing.map(r=>{
