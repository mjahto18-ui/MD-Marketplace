...
export default function DonateClient({ pendingId }){
  ...
  const send = async (amount, charity) => {
    ...
    alert(charity ? `تبرعت ${amount.toLocaleString()} لـ ${charity} ورجع ${net-amount} محفظتك` : `رجع كلو ${net.toLocaleString()} عالمحفظة`)
  }
      ...
      <div className="w-full max-w-5xl lg:flex gap-6">
        <div className="lg:w-1/3 bg-white rounded-2xl p-5 border shadow-sm h-fit lg:sticky lg:top-4">
          ...
          <button onClick={()=>send(donate, sel)} disabled={donate>0 && !sel} className="w-full bg-black text-white p-4 rounded-xl mt-4 font-bold disabled:opacity-30">
            {donate>0 ? (sel ? `تبرع ${donate.toLocaleString()} لـ ${sel}` : 'اختر جمعية للمساهمة') : 'اختر جمعية للمساهمة'}
          </button>
