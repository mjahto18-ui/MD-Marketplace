...
export default function CartPage() {
  ...
  const updateQty = async (cartID, newQty) => {
    if (globalCfg?.isCartClosed || globalCfg?.isComingSoon) return;
    if (newQty < 1) return;
  ...
  const isCartBlocked = globalCfg?.isCartClosed || globalCfg?.isComingSoon;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-slate-900 to-slate-950 text-white" style={{ direction: 'rtl' }}>
      {/* بانر السلة المغلقة - الرسالة الديناميك من الشيت */}
      {isCartBlocked && (
        <div className="bg-amber-500 text-black text-center py-3 px-4 font-bold sticky top-0 z-50">
          {globalCfg.cart_closed_message || globalCfg.cart_enabled?.message || "السلة مغلقة حالياً"}
        </div>
      )}
