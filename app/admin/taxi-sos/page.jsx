...
import { Phone, Car, User, CheckCircle, MapPin, Timer } from "lucide-react";
            ...
            alerts.map((a) => (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text- text-white/40 mono">{a.created_at? new Date(a.created_at).toLocaleTimeString('ar-LB', {hour:'2-digit', minute:'2-digit'}) : ''}</span>
                </div>
                ...
                {a.comment && <div className="text- bg-black/40 border border-white/5 text-amber-300 rounded-lg p-1.5 mt-2 truncate">💬 "{a.comment}"</div>}
              </div>
            ))
          )}
        </div>
        ...
        <div className="col-span-1 lg:col-span-3 space-y-6">
          {selectedAlert? (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className="xl:col-span-1 bg-[#141414] border border-white/[0.06] rounded- p-6 space-y-5">
                ...
                <div className="border-t border-white/[0.04] pt-4 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div><div className="text- text-white/40">الزبونة (الركاب)</div><div className="text-sm font-bold text-white">{selectedAlert.customer_name}</div><div className="text-xs text-white/60 font-mono mt-0.5">{selectedAlert.customer_phone}</div></div>
                  </div>
                  <div className="flex items-center gap-3">
              ...
              {/* هذا هو الجزء اللي كان ناقص عندك */}
              <div className="xl:col-span-2 bg-[#141414] border border-white/[0.06] rounded- overflow-hidden flex flex-col h-">
                ...
                <div className="flex-1 relative bg-[#0a0a0a]">
                  {selectedAlert.lat && selectedAlert.lng? (
                    <Map lat={selectedAlert.lat} lng={selectedAlert.lng} driverName={selectedAlert.driver_name} />
                  ) : (
                ...
                <div className="p-3 bg-black/30 border-t border-white/[0.06] flex justify-between text- text-white/40 mono">
                  <span>LAT: {selectedAlert.lat || '--'}</span>
                  <span>LNG: {selectedAlert.lng || '--'}</span>
